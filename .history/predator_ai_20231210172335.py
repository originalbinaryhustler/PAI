...
def get_hash_values():
   ...
   print('If your password is not found, try a different wordlist.')
...
def crack_hash(hash_value: str, wordlist_input: str) -> str:
 """
 Function to crack a hash value using a wordlist.
 Parameters:
 - hash_value: str
    The hash value that needs to be cracked.
 - wordlist_input: str
    The path to the wordlist file or a URL to download the wordlist.
 Returns:
 - str:
    The word from the wordlist that matches the given hash value.
    Returns an empty string if no match is found.
 Raises:
 - TypeError:
    Raises an error if the hash_value is not a string or if the wordlist_input is not a string.
 - FileNotFoundError:
    Raises an error if the wordlist file does not exist.
 """
 # Checking if the input parameters are of the correct types
 ...
 if wordlist_input.startswith('http://') or wordlist_input.startswith('https://'):
    try:
        response = requests.get(wordlist_input)
        response.raise_for_status()
        wordlist_file = 'wordlist.txt'
        with open(wordlist_file, 'w') as file:
            file.write(response.text)
    except requests.HTTPError as e:
        print(f"Error: An HTTP error occurred while downloading the wordlist: {e}")
        return ""
    except Exception as e:
 ...
 return printr(f"The cracked word for hash {hash_to_crack} is: {cracked_word}")
